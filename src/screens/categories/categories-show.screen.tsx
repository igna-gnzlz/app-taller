...
import { getNutriScoreColor } from "@/src/utils/getNutriScoreColor";
...
export function CategoriasShowScreen() {
  const { id } = useLocalSearchParams<CategoriaParams>();
  const {
    data,
    hasNextPage,
    fetchNextPage,
  ...
  } = useInfiniteProductosByCategoria(id);
  ...
  // console.log(productosCombinados2.map((p) => p.id));
